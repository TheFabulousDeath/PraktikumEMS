...
def process_csv(file_path, temperature_k=288.15, accelerometer_sensitivity=4096, time_interval_ms=10):
    ...
    # Create a time sequence based on the time interval
    time = np.arange(0, len(data) * time_interval_ms, time_interval_ms) / 1000  # Convert to seconds
    ...
    z_cumsum = z.cumsum().cumsum()/1000

    # Create traces for each column
    trace_pressure = go.Scatter(x=time, y=pressure, mode='lines', name='Pressure')
    trace_x = go.Scatter(x=time, y=x, mode='lines', name='X')
    trace_y = go.Scatter(x=time, y=y, mode='lines', name='Y')
    trace_z = go.Scatter(x=time, y=z, mode='lines', name='Z')
    trace_r = go.Scatter(x=time, y=z_cumsum, mode='lines', name='R (Double Cumulative Sum of Z)')
    trace_altitude = go.Scatter(x=time, y=altitude_diff, mode='lines', name='Altitude Difference')
